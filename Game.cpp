...
Game::~Game() {
	...
	cout << "Destroyed SDL Resources. Quitting..." << endl;
}
...
void Game::Run() {
	...
	// When the loop is completeted,
	// the class will be destroyed
	...
	while (SDL_PollEvent(&ev)) {
		...
		if (ev.type == SDL_KEYDOWN) {
			...
			if (ev.key.keysym.sym == SDLK_SPACE) {
				...
				if (isRunningSim)		SDL_SetWindowTitle(window, "The Game of Life - SDL2 --- (running)");
				else if (!isRunningSim)	SDL_SetWindowTitle(window, "The Game of Life - SDL2 --- (paused)");
			}
		}
	}
...
void Game::Draw() {
	for (int x = 0; x < grid.rows; x++) {
		for (int y = 0; y < grid.cols; y++) {
			...
			if (cells[x][y].isAlive) SDL_SetRenderDrawColor(renderer, 20, 20, 20, 255);
	...
	for (int i = 0; i <= grid.cols; i++) {
		SDL_FRect hLine = {
			grid.posX + 1,
			grid.posY + (i * grid.cellHeight) + 1,
			(i == grid.cols) ? grid.width + 2 : grid.width,
			2
		};
